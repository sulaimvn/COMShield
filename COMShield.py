...
def run_recmd(sid):
    ...
    for user in os.listdir(os.path.join(os.path.dirname(__file__), "output", "userprofile")):
        
        input_file_path = os.path.join(destination_directory, user, "UsrClass.dat")

        output_file_path = os.path.join(destination_directory, user, sid, "UsrClass_CLSID.txt")
        
        os.makedirs(os.path.dirname(output_file_path), exist_ok=True)
        
        command = r"{} -f {} --kn {}_Classes\CLSID\ --nl".format(recmd_path, input_file_path, sid)
        
        subprocess.run(command, shell=True, stdout=open(output_file_path, "w"))
...
def compare_registry_values(fname):
    ...
    # Read HKLM file and populate hklm_contents dictionary
    ...
    # Read all users file and compare with hklm_contents
    differences = []
    ...
    # Write differences to output CSV file
...
def main():
 ...
 '''
    ...
    parser = argparse.ArgumentParser(prog='COMShield', usage='\t\tpython.exe COMShield.py [options]', description='COMShield tool for Windows compares the registry values of a specific key between HKEY_USERS (HKU) and HKEY_LOCAL_MACHINE (HKLM) in the CLSID registry. It detects any differences between the two and alerts the user if any discrepancies are found. \n\n Emaples:\n\n'
          '\tpython.exe %(prog)s \n'
          '\tpython.exe %(prog)s --output output.csv\n'
